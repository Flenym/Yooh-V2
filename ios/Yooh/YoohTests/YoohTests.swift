...
/// Unit tests for backend-coupled logic: wire codec, validation parity
/// and JSON decoding of real server shapes. Run: Product → Test.
final class YoohWireTests: XCTestCase {

    // MARK: - Engine.IO polling framing (v6: RS-separated)
    ...
    // MARK: - Validation parity (utils.js / zod)
    ...
    func testScheduledMessageDecoding() throws {
        let json = """
        {"id":"m9","chatId":"c1","senderId":"u1","type":"text","text":"later",
         "stream":"main","reactions":[],"readByUserIds":[],
         "createdAt":"2026-09-01T10:00:00.000Z","scheduledAt":"2026-09-02T10:00:00.000Z"}
        """
        let m = try JSONDecoder().decode(YoohMessage.self, from: Data(json.utf8))
        XCTAssertEqual(m.scheduledAt, "2026-09-02T10:00:00.000Z")
    }

    func testUserDecoding() throws {
        let json = """
        {"id":"u1","chatId":"1234567890","phone":"+10000000000","username":"alice",
         "displayName":"Alice","birthday":"1990-05-01","locale":"en",
         "cloudPasswordEnabled":false,"isBot":false,"isSystemBot":false,"isPremium":false,
         "starsBalance":5,"starsEarned":5,"emojiStatus":"","feedbackBlocked":false}
        """
        let u = try JSONDecoder().decode(YoohUser.self, from: Data(json.utf8))
        XCTAssertEqual(u.birthday, "1990-05-01")
        XCTAssertEqual(u.starsBalance, 5)
    }
