...
/// Device-local secret chats — the same semantics as the web client:
/// messages never touch the server, an optional self-destruct timer
/// deletes them, and a fingerprint identifies the peer pairing.
/// Honestly local: clearly labeled as device-only in the UI.
struct SecretChat: Codable, Identifiable, Hashable {
    ...
    /// Deterministic pairing fingerprint (order-independent).
...
/// Local persistence (per user), mirroring the web client's localStorage
/// secret-chat maps. Nothing here ever leaves the device.
final class SecretStore: @unchecked Sendable {
    ...
    /// Live (non-expired) messages, oldest first; expired ones are pruned.
