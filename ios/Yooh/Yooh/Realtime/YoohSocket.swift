...
/// Every realtime event the server can push (see realtime.js + the
/// `emitRealtimeEvent` bridge in index.js). Payloads decode tolerantly:
/// unknown future events are ignored, never crash the socket.
...
/// Call signaling payloads (payload shapes from realtime.js).
struct CallSignal {
    enum Kind: String {
...
/// Owns the realtime connection: Engine.IO transport + Socket.IO framing +
/// Yooh domain events + reconnect policy.
///
/// Mirrors the web client's `connectSocket()`:
/// - auth `{ token }` in the namespace CONNECT packet
/// - infinite reconnect with capped backoff
/// - explicit `chat:join` per chat after every (re)connect
/// - `syncAppState`-equivalent is the delegate's job (REST refresh)
final class YoohSocket {
    ...
    init() {
        ...
        transport.onState = { [weak self] state in
            ...
            // First transport open per Engine.IO session → namespace CONNECT
            // carrying `auth: { token }` (server reads handshake.auth.token).
    ...
    private func handleEnginePacket(_ packet: EnginePacket) {
        ...
        case "4": // Socket.IO message
            ...
            handleSIOPacket(sio)
        case "1": // transport close → reconnect
    ...
    private func handleSIOPacket(_ sio: SIOPacket) {
        switch sio.type {
        case "0": // CONNECT confirmed (namespace joined, auth accepted)
            sioConnected = true
            reconnectAttempt = 0
            state = .connected
        ...
        case "2": // EVENT
            ...
            // Server→client events never carry ack ids we must answer.
            dispatchEvent(name: name, args: Array(sio.args.dropFirst()))
        case "3": // ACK for one of our emits
            if let id = sio.ackId {
                let payload = sio.args.first
                resolveAck(id: id, payload: payload)
            }
        case "4": // CONNECT_ERROR (e.g. bad/expired token)
            ...
            if message.lowercased().contains("invalid") || message.contains("401") {
                // Dead token — stop retrying, let SessionStore log out.
                reconnectTask?.cancel()
                reconnectTask = nil
                api.onUnauthorized?()
            }
    ...
    private func scheduleReconnect() {
        ...
        reconnectAttempt += 1
        // Web parity: 900ms base, 5s cap (iPhone profile: 500ms / 3.2s).
    ...
    /// Fire-and-forget emit (chat:join, chat:typing).
    func emit(name: String, payload: Any?) {
        do {
            ...
            #if DEBUG
            log.error("emit encode failed: \(error.localizedDescription, privacy: .public)")
            #endif
        }
    }

    /// Emit with server ack (chat:read, call:*). Resolves with the ack payload.
    ...
    // MARK: - Domain emits (client → server)

    /// NOTE: the server expects a bare string chatId, not an object.
    ...
    // Calls (signaling only; media needs a WebRTC engine — see CallService).
    ...
    // MARK: - Namespace connect

    /// Sends `40{"token":...}` — called automatically once the transport opens.
