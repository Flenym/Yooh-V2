...
/// Engine.IO v4 transport with long-polling + opportunistic WebSocket upgrade.
///
/// - Handshake and steady-state run on HTTP long-polling (works everywhere,
///   including constrained networks).
/// - Immediately after the handshake, the client attempts a WebSocket upgrade
///   (`2probe` → `3probe` → `5`); live traffic moves to the socket and the
///   poll loop stops. Any WS failure falls back to polling transparently.
/// - Server pings (`2`) are answered with pongs (`3`) on the active transport.
/// - Silence longer than pingInterval + pingTimeout triggers a fresh handshake.
final class EngineIOClient {
    ...
    /// Enqueues a raw Engine.IO packet (`40...`, `42[...]`, `3`, ...).
    ...
    private func performHandshake() {
        ...
        applyHeaders(to: &req)
        #if DEBUG
        log.debug("EIO handshake \(url.absoluteString, privacy: .public)")
        #endif
        session.dataTask(with: req) { [weak self] data, _, error in
            ...
            self.startWatchdog()
            // Surface any non-open packets bundled with the handshake.
            for p in packets where p.type != "0" { self.handlePacket(p) }
            // Opportunistic WS upgrade, then start polling as the carrier.
    ...
    private func longPoll() {
        ...
        let task = session.dataTask(with: req) { [weak self] data, _, error in
            ...
            if let error as NSError?, error.code != NSURLErrorCancelled {
                ...
                #if DEBUG
                self.log.debug("poll error, re-issuing: \(error.localizedDescription, privacy: .public)")
                #endif
                self.longPoll()
                return
            }
    ...
    private func attemptWebSocketUpgrade() {
        ...
        lock.unlock()
        #if DEBUG
        log.debug("EIO ws probe \(url.absoluteString, privacy: .public)")
        #endif
        ws.resume()
        ...
        ws.receive { [weak self] result in
            ...
            case .success(let msg):
                ...
                if expectingProbe {
                    ...
                    // Drain anything queued during probing over the socket.
                    ...
                    #if DEBUG
                    self.log.debug("EIO upgraded to websocket")
                    #endif
    ...
    private func webSocketFailed() {
        ...
        wsUpgraded = false
        if wasUpgraded {
            // Live socket died → full re-handshake (server re-joins rooms on
            // connect; YoohSocket replays chat:join + REST sync).
            setStateLocked(.connecting)
            lock.unlock()
            #if DEBUG
            log.debug("EIO ws lost, re-handshaking")
            #endif
            performHandshake()
        } else {
            // Probe failed → stay on polling.
            setStateLocked(.open(transport: .polling))
            lock.unlock()
            #if DEBUG
            log.debug("EIO ws probe failed, staying on polling")
            #endif
        }
    }
    ...
    private func handlePacket(_ p: EnginePacket) {
        touchReceive()
        switch p.type {
        case "2": // ping → pong on the same transport
            send(EnginePacket(type: "3", body: ""))
        case "1": // server close → reconnect cleanly
    ...
    private func startWatchdog() {
        ...
        timer.setEventHandler { [weak self] in
            ...
            #if DEBUG
            self.log.debug("EIO watchdog fired (\(Int(silentFor))s silent), re-handshaking")
            #endif
            self.lock.lock()
            self.pollTask?.cancel(); self.pollTask = nil
    ...
    private func fail(_ error: Error) {
        ...
        #endif
        // Notify as a synthetic close; YoohSocket owns retry policy.
