...
/// Login / register flow state machine (OTP-only, per server).
///
/// Login: identifier(phone|email) → code → [cloud-password] → session
/// Register: phone → code+profile(displayName,username) → session
@Observable
@MainActor
final class AuthViewModel {
    ...
    var identifier = ""       // phone or email (login)
