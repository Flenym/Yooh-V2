...
/// Remote attachment thumbnail with memory+disk cache (see ImageCache).
/// Needs the auth token because `/api/files/...` uses `?token=` auth.
struct CachedFileImage: View {
    ...
    private func load() async {
        ...
        do {
            ...
            // Thumbnail failure degrades to the file row (never a blank crash).
        }
    }
}
