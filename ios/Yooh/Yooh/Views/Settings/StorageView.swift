...
/// Data & storage: real on-device usage, cache clearing and
/// server-persisted auto-download preferences (`settings.data`).
struct StorageView: View {
    ...
    private func measure() async {
        imageCacheBytes = await directoryBytes(subpath: "yooh-images", in: .cachesDirectory)
        tmpBytes = await directoryBytes(subpath: nil, in: nil)
    }
