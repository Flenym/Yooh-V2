...
/// Support tickets (real API): state, creation per category and jumping
/// into the support conversation.
...
/// Short FAQ compiled from the product docs (no backend needed).
struct FAQView: View {
    private let items: [(q: String, a: String)] = [
        ("How do I log in?", "Use your phone number or email. Enter the 6-digit code from SMS/email (in dev builds the code is also visible in the admin panel). If cloud password is on, enter it on the second step."),
