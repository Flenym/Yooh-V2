...
/// One backend call. Every route exists in src/server/app.js — no invented APIs.
struct APIEndpoint {
    var method: HTTPMethod = .get
    var path: String
    var query: [URLQueryItem] = []
    var jsonBody: (any Encodable)?
    /// Admin token (`x-admin-token`) instead of the user JWT. Admin console only.
    ...
    static func syncContacts(phones: [String]) -> APIEndpoint {
        ...
        return APIEndpoint(method: .post, path: "/api/me/contacts",
