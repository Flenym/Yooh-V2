...
/// two-line preview (colored sender prefix in groups), unread badge.
struct ChatRowView: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel(Text("\(title). \(previewPlain)"))
    }
    ...
    private var statusColumn: some View {
        VStack(alignment: .trailing, spacing: 6) {
            if chat.hasUnread(myUserId: myUserId) {
                Circle()
                    .fill(isMuted ? YoohTheme.TG.badgeMuted : ThemeStore.shared.accent)
