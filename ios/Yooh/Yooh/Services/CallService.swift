...
/// Call history (`GET /api/calls`) + signaling passthrough to YoohSocket.
///
/// Media scope (explicit, honest): live audio/video needs a WebRTC engine
/// (e.g. WebRTC.xcframework + TURN from `GET /api/webrtc/config`). That
/// native dependency is intentionally NOT vendored in v1 — history,
/// incoming-call alerts and signaling hooks are real; the in-call media
/// screen is marked TODO and surfaced as "unavailable" rather than faked.
/// See ios/README-iOS.md § "Calls".
final class CallService {
    ...
    // MARK: - Signaling (real socket events, usable once media lands)
